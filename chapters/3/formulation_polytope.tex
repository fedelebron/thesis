...
Lastly, the difference between $P(I)$ and $P_{\R}(I)$ is that, in $P(I)$, we require $x_{c, l, p, k}$, $class\_date_{c, l, d}$, $busy\_l_{p, c, l}$, $busy_{p, c, d}$, $cp_{c, p}$, $csd_{c, sd}$ to all be in $\Z$.
